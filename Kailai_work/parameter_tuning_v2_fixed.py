...
class ParameterOptimizer:
    ...
    def _extract_metrics_from_notebook(self, cells):
        ...
        model_type = self._get_model_type(self.notebook_path)
        ...
        if is_classification:
            ...
            print("Initialized with regression metrics keys. Will extract actual metrics...")
            result = {
        ...
        # Track if we've found actual metrics (vs. using synthetic defaults)
        found_metrics = False
        metrics_by_model = {}
        
        # First look for instrumentation metrics - these are added by our code
        for cell in cells:
            if cell.cell_type != 'code':
                continue
            ...
            for output in cell['outputs']:
                ...
                param_tuning_metrics = re.findall(r'PARAMETER_TUNING_METRIC:\s*(\w+)=([0-9.e+-]+)', text)
                if param_tuning_metrics:
                    for metric_name, value_str in param_tuning_metrics:
                        try:
                            value = float(value_str)
                            result[metric_name] = value
                            found_metrics = True
                            print(f"Found instrumented metric: {metric_name}={value}")
                        except (ValueError, KeyError):
                            continue
        
        # If instrumentation didn't yield metrics, use pattern matching
        if not found_metrics:
            print("No instrumentation metrics found. Falling back to extraction from code patterns.")
            
            # Extract from code cells
            ast_extractor = MetricsExtractor()
            
            for cell in cells:
                if cell.cell_type != 'code':
                    continue
                
                # Parse the Python code with AST
                try:
                    code = cell.source
                    tree = ast.parse(code)
                    ast_extractor.visit(tree)
                except Exception:
            ...
            # Process metrics collected by AST and output pattern matching
            metrics = ast_extractor.metrics
            self._process_metrics_dictionary(metrics_by_model)
            
            # Integrate metrics from AST extractor
            ...
            # Integrate metrics from output pattern matching
        ...
        # Check for metrics specific to notebook type
        if is_classification:
        ...
        if not_found_metrics:
            print(f"Metrics not found in notebook, keeping as None: {', '.join(not_found_metrics)}")
            
        return result
    
    def _get_model_type(self, notebook_path):
        """
        Determine the model type based on indicators in the notebook.
        ...
        """
        # First check notebook path/name
        model_type = self._get_model_type_by_name(notebook_path)
        
        # Initialize score counters for different model types
        model_scores = {
            'xgboost': 0,
            'randomforest': 0,
            'neural_network': 0,
        ...
        # Check cells for model type indicators
        for cell in self.nb.cells:
            ...
            # Check for different model types in the code
            if 'xgboost' in code or 'xgb' in code:
                model_scores['xgboost'] += 3
            if 'randomforest' in code or 'random_forest' in code:
                model_scores['randomforest'] += 2
            if any(term in code for term in ['keras', 'tensorflow', 'nn.', 'sequential', 'dense']):
                model_scores['neural_network'] += 3
            if 'logisticregression' in code or 'logistic_regression' in code:
                model_scores['logistic_regression'] += 2
            if 'linearregression' in code or 'linear_regression' in code:
                model_scores['linear_regression'] += 2
            if 'svm' in code or 'svc' in code or 'svr' in code:
                model_scores['svm'] += 2
            if 'decisiontree' in code or 'decision_tree' in code:
                model_scores['decision_tree'] += 2
            if 'knn' in code or 'kneighbors' in code or 'nearest' in code:
                model_scores['knn'] += 2
                
        # Find model with highest score
        max_score = 0
        detected_model = model_type  # Default to name-based detection
        ...
        # If no clear model detected from code, use name-based detection
        if max_score == 0:
            return model_type
    ...
    def _is_classification_notebook(self, notebook_path):
        """
        Determine if a notebook is for classification or regression based on the code.
        ...
        """
        # First check notebook name
        name_classification = self._is_notebook_classification_by_name(notebook_path)
        
        # Lists of indicators for classification and regression
        classification_indicators = [
            ...
            'sigmoid', 'softmax', 'Binary', 'MultiClass', 'categorical'
        ]
        
        regression_indicators = [
            'mean_squared_error', 'mean_absolute_error', 'r2_score',
            'root_mean_squared_error', 'rmse', 'mse', 'mae', 'r2',
            'mean_squared_log_error', 'median_absolute_error',
            'explained_variance_score', 'max_error',
            'regression', 'regressor', 'predict continuous',
            'mape', 'mean_absolute_percentage_error',
            'huber_loss', 'mean_squared_logarithmic_error',
            'cosine_similarity', 'mean_poisson_deviance'
        ]
        ...
        for cell in self.nb.cells:
            ...
            # Count regression indicators
            for indicator in regression_indicators:
                if indicator.lower() in code:
                    regression_score += 1
        
        # Add weight from name-based classification
        if name_classification:
            classification_score += 2
        else:
            regression_score += 2
            
        # Determine type based on indicator scores
